...
func getEnvDefault(key, def string) string { v := os.Getenv(key); if v == "" { return def }; return v }
...
func mustInt(key string) int {
	...
	if err != nil { panic(key + " must be an integer") }
	return v
}
func parseDuration(s string) time.Duration {
	d, err := time.ParseDuration(s); if err != nil { panic(err) }
	return d
}
